...
def adivinafig(PreguntaActiva):
    ...
    if Ruta.exists():
        ...
        with Ruta.open() as archivojson:
            pregres = json.load(archivojson)
        Preguntas = list(pregres.values())
        Dato = filter(lambda e: e[0] == PreguntaActiva,Preguntas[0])
        LDato = list(Dato)[0]
        print(LDato[1])
        Resp = input(LDato[2])
        if (LDato[2] != "fin"):
            Dato = filter(lambda e: e[0] == Resp, Preguntas[1])
            LDato = list(Dato)[0]
            PreguntaActiva = LDato[1]
