...
def jueguito(carac,preg,Lanimales,C):
    animalito = ""
    clasesitas = list(C.keys())
    animalitos = list(C.values())
    #print(clasesitas)
    #print(animalitos)
    print("Describe la caracteristicas de tu animalito elegido")
    rej = genera_respuesta(carac,preg)
    #resj = [(str(rej),str(Lanimales[a]))]
    cc=0
    for c in clasesitas:
        if(rej == clasesitas[cc]):
            #print(animalitos[cc])
            animalito = animalitos[cc]
        else:
            cc += 1
    #print("ESTO TIENE EL DICCIONARIO POR AHORA: \n",clases)
    return animalito
...
def main():
    ...
    if Ruta.exists():
        ...
        with Ruta.open() as archivojson:
            ...
            ################################# JUEGUITO
            print("\n\n\n\(-_-)/ Bienvenido al Juegito de Adivina tu animalito \(-_-)/")
            print("\nPiensa en uno de estos animales: \n", Lanimales)
            AnimalAdivinado = jueguito(carac,preg,Lanimales,C)
            print("\n\n****** TU ANIMALITO ES...    ", AnimalAdivinado, "    *******")
